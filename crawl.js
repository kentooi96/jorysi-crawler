...
// 打开一个页面并尝试通过验证页。返回 { ok, challenged }。失败不抛异常。
async function gotoPage(page, url, label) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const challenged = isChallengeText(body);
      if (attempt === 0) {
        console.log(`  [${label}] ${challenged ? "卡在验证页" : "内容未就绪"}，重试一次…`);
        await page.waitForTimeout(3000);
        try { await page.reload({ waitUntil: "domcontentloaded", timeout: 40000 }); } catch (_) {}
      } else {
